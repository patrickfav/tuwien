...
import server.MetaServiceServer;
import constants.CRUDAction;
import exceptions.HttpContentException;

public class LookupLocationHttpRequestHandler extends AbstractRestHandler {
	...
	private static String META_SERVICE_PRIVATEKEY="keys/metaservice.pem";
	private static String EXTERNAL_SERVICE_PUBLICKEY="keys/externalservice.pub.pem";
	
	public static SecureChannelAsym scAsymMetaService;
	
	public LookupLocationHttpRequestHandler() {
		try {
			if(scAsymMetaService == null)
				scAsymMetaService = new SecureChannelAsym(META_SERVICE_PRIVATEKEY,EXTERNAL_SERVICE_PUBLICKEY);
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void handle(String arg0, Request baseRequest,HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		try {
			if(r.getCrudAction().equals(CRUDAction.LOCATIONS)) {
				log.info("Got valid lookup request.");
				LookupResponse lookupResponse = new LookupResponse();
				lookupResponse.setLocations(MetaServiceServer.getLocations());
				ByteArrayOutputStream f = new ByteArrayOutputStream();
				JAXB.marshal(lookupResponse, f);
				response.setContentType("text/html;charset=utf-8");
				response.setStatus(HttpServletResponse.SC_OK);
				response.getWriter().println(LookupLocationHttpRequestHandler.scAsymMetaService.encryptRSAWithServerPublicKey(f.toString("utf-8")));
				f.close();
	...
	private RestRequest analyseRequest(String requestUrl, String method,HttpServletRequest request) throws MalformedURLException, HttpContentException {
		...
		if(method.equals(CRUDAction.READ) && (urlParts.length == 1) && urlParts[0].equals(MetaServiceServer.LOOKUP_URL_KEYWORD)) {
			r.setCrudAction(CRUDAction.LOCATIONS);	
		} else {
