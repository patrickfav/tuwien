...
public class DateTimeCellRenderer extends JLabel implements TableCellRenderer {
	...
	public Component getTableCellRendererComponent(JTable table, Object value, boolean isSelected, boolean hasFocus, int row, int column) {
		...
		setText(numberWithLeadingZero(c.get(Calendar.DAY_OF_MONTH),2)+"."+numberWithLeadingZero((c.get(Calendar.MONTH)+1),2)+"."+c.get(Calendar.YEAR)+" / "+numberWithLeadingZero(c.get(Calendar.HOUR_OF_DAY),2)+":"+numberWithLeadingZero(c.get(Calendar.MINUTE),2));
